...
```bash
pm2 start "remoteagent start my-app" --name my-app
```
...
<Tip>
  If you have multiple agents (Pro plan), start each one by name: `pm2 start "remoteagent start project-a" --name project-a` and `pm2 start "remoteagent start project-b" --name project-b`.
</Tip>

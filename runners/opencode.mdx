...
[OpenCode](https://opencode.ai) is an open-source terminal-based AI coding agent built by the SST team. It is the **default recommended runner** for RemoteAgent. The `opencode` runner invokes `opencode run` as a subprocess and supports over 75 AI providers — including GitHub Copilot and local models via Ollama — with no API key required to get started.
...
Or choose `opencode` when prompted by the interactive wizard (it is listed first as the recommended default).
...
RemoteAgent injects configuration at runtime using the `OPENCODE_CONFIG_CONTENT` environment variable — opencode's native config injection mechanism. RemoteAgent does not write any config files to disk.
...
Only the settings collected during the wizard are injected. Any additional opencode configuration you set in your own `opencode.json` file will be picked up by opencode automatically — RemoteAgent does not interfere with it.

...
<Warning>
  ...
  both the OAuth token and `claude login`, even if you have an active Pro/Max plan.
  If you want to authenticate via subscription, make sure `ANTHROPIC_API_KEY` is
  **not set** in your environment (or in `~/.zshrc` / `~/.bash_profile`).
</Warning>

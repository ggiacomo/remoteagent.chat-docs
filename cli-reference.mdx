...
### `remoteagent pair [name]`

Re-pair an existing local agent with a new dashboard entry. Useful if the agent was deleted from the dashboard or the pairing expired.

```bash
remoteagent pair
remoteagent pair my-project
```

The command stops the agent (if running), prompts you for a new pairing code from the [dashboard](https://remoteagent.chat/dashboard), and updates the local config without touching runner settings.
...
Remove all agents, delete `~/.remoteagent/` (all configs and logs), and remove the `remoteagent` binary.

```bash
remoteagent uninstall
```

Asks for confirmation before doing anything. Stops and deregisters all agents from the server first.

<Warning>
  This removes everything. To start using RemoteAgent again you will need to
  reinstall and run `remoteagent init`.
</Warning>
